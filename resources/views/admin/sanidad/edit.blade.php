...
<div class="p-6">
  <div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6">
      <form method="POST" action="{{ route('admin.sanidad.update', $treatment->IDSanidad) }}" class="space-y-4">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="date" name="Fecha" value="{{ old('Fecha', optional($treatment->Fecha)->toDateString()) }}" class="w-full border-gray-300 rounded-md" required />
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="Producto" value="{{ old('Producto', $treatment->Producto) }}" class="w-full border-gray-300 rounded-md" maxlength="100" required />
          </div>
          <div>
        ...
        <div>
          ...
          <input type="text" name="Dosis" value="{{ old('Dosis', $treatment->Dosis) }}" class="w-full border-gray-300 rounded-md" maxlength="50" />
        </div>

        <div>
          <label class="block text-sm text-gray-600 mb-1">Observaciones</label>
          <textarea name="Observaciones" rows="3" class="w-full border-gray-300 rounded-md">{{ old('Observaciones', $treatment->Observaciones) }}</textarea>
        </div>
