...
<div class="p-6">
  <div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Registrar Tratamiento</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      <form method="POST" action="{{ route(request()->routeIs('veterinario.*') ? 'veterinario.sanidad.store' : 'admin.sanidad.store') }}" class="space-y-4">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="date" name="Fecha" value="{{ old('Fecha', now()->toDateString()) }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md" required />
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="Producto" value="{{ old('Producto') }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md" maxlength="100" required />
          </div>
          <div>
        ...
        <div>
          ...
          <input type="text" name="Dosis" value="{{ old('Dosis') }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md" maxlength="50" />
        </div>

        <div>
          <label class="block text-sm text-gray-600 dark:text-gray-300 mb-1">Observaciones</label>
          <textarea name="Observaciones" rows="3" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md">{{ old('Observaciones') }}</textarea>
        </div>
